...
import { COMPANIES_BY_COUNTRY } from '../../services/mock/ledgers'
...
export function LedgerList() {
  ...
  // Reset company filter and page when country changes
  useEffect(() => {
    setCompanyFilter('')
    setPage(1)
  }, [selectedCountry])
  ...
  const companyOptions = [
    ...
    ...(COMPANIES_BY_COUNTRY[selectedCountry] ?? []),
  ]
  ...
  const filtered = ledgers.filter(l => {
    ...
    return matchSearch && matchStatus && matchCompany && matchCountry
  })
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="w-40">
            <Select
              ...
              value={statusFilter}
              onChange={e => { setStatusFilter(e.target.value as LedgerStatus | ''); resetPage() }}
            />
          </div>
          <div className="w-48">
            <Select
      ...
      <Card padding={false}>
        {filtered.length === 0 ? (
          ...
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr style={{ background: '#F1F2F6', borderBottom: '2px solid #F1F2F6' }}>
                    ...
                    <th className="text-left px-4 py-3 text-sm font-semibold" style={{ color: '#121E6C' }}>Compañía</th>
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        <span className="text-sm" style={{ color: '#606060' }}>
                          {ledger.company === 'CF' ? 'Bold CF'
                            : ledger.company === 'SAS' ? 'Bold SAS'
                            : ledger.company === 'Capital' ? 'Bold Capital'
                            : 'Bold Peru'}
                        </span>
                      </td>
                      <td className="px-4 py-4">
            ...
            >
              <span className="text-sm" style={{ color: '#606060' }}>
                Página {page} de {totalPages} · {filtered.length} ledgers
              </span>
